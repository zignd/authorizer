...
(s/defn ^:private to-minutes-from-epoch :- s/Int
  "Converts an OffsetDateTime to epoch in minutes."
  [date-time :- OffsetDateTime]
  (int (/ (jtime/to-millis-from-epoch date-time) 60000)))

(s/defn ^:private to-minutes-list :- [s/Int]
  "Given an account and a transaction, returns a list containing the time of every
   transaction in the account's history and the time of the separately provided transaction
   in minutes and sorted."
  [account :- Account
...
(s/defn validate-doubled-transaction :- (s/maybe s/Keyword)
  "There should not be more than 1 similar transactions
   (same amount and merchant) in a 2 minutes interval."
  [account :- Account
   transaction :- Transaction]
